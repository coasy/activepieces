...
- **`enrollUserCourse`** writes a `UserCourse` progress row; access itself comes from `user.courses`. Without `directGrant: true` the course stays `LOCKED` in the member app, and `ALL_COURSES` cannot cover a `PRIVATE` course. The piece sends `directGrant` (default on).
